...
use wpiutil_sys::{
    WPI_DataLog, WPI_DataLog_AppendBoolean, WPI_DataLog_AppendBooleanArrayByte,
    WPI_DataLog_AppendDouble, WPI_DataLog_AppendDoubleArray, WPI_DataLog_AppendFloat,
    WPI_DataLog_AppendFloatArray, WPI_DataLog_AppendInteger, WPI_DataLog_AppendIntegerArray,
    WPI_DataLog_AppendRaw, WPI_DataLog_AppendString, WPI_DataLog_AppendStringArray,
    WPI_DataLog_CreateBackgroundWriter, WPI_DataLog_CreateWriter, WPI_DataLog_Finish,
    WPI_DataLog_Flush, WPI_DataLog_Pause, WPI_DataLog_Release,
    WPI_DataLog_SetBackgroundWriterFilename, WPI_DataLog_SetMetadata, WPI_DataLog_Start,
    WPI_DataLog_Stop,
};
...
fn check_utf8<'a>(s: Option<&'a str>) -> Result<&'a str, std::io::Error> {
    match s {
        ...
        None => Err(std::io::Error::new(
            ...
            "Filename or directory is not UTF-8",
        )),
    }
}
...
impl<T: DataLogKind> DataLog<T> {
    ...
    ) -> Result<DataLog<Writer>, std::io::Error> {
    ...
    pub fn new_background(
        ...
        period: f64,
        extra_header: Option<&str>,
    ) -> Result<DataLog<BackgroundWriter>, std::io::Error> {
        let filename_wpi_str = WPIString::from_str(check_utf8(
            filename.map_or_else(|| Some(""), |m| m.to_str()),
        )?);
    ...
    pub fn flush(&self) {
        unsafe {
            WPI_DataLog_Flush(self.p);
        }
    }

    pub fn pause(&self) {
        unsafe {
            WPI_DataLog_Pause(self.p);
        }
    }
...
impl<'a, T: DataLogKind> DataLogEntry<'a, T> {
    ...
    pub fn append_string(&self, value: &str, timestamp: Option<i64>) {
        unsafe {
            WPI_DataLog_AppendString(
                self.parent.p,
                self.index,
                WPIString::from_str(value).as_raw(),
                timestamp.unwrap_or(0),
            );
        }
    }
    ...
    pub fn append_str_array(&self, value: &[&str], timestamp: Option<i64>) {
        let v: Vec<wpiutil_sys::WPI_String> = value
            .iter()
            .map(|s| wpiutil_sys::WPI_String {
