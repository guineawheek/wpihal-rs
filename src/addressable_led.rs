use wpihal_sys::{
    HAL_AddressableLEDData, HAL_AddressableLEDHandle, HAL_FreeAddressableLED,
    HAL_InitializeAddressableLED, HAL_SetAddressableLEDBitTiming, HAL_SetAddressableLEDLength,
    HAL_SetAddressableLEDSyncTime, HAL_StartAddressableLEDOutput, HAL_StopAddressableLEDOutput,
    HAL_WriteAddressableLEDData,
};

use crate::{Handle, dio::DIO, error::HALResult, hal_call};
...
impl<'a> AddressableLED<'a> {
    ...
    pub fn initialize(output_port: &'a DIO) -> HALResult<Self> {
        ...
        Ok(Self(
            hal_call!(HAL_InitializeAddressableLED(output_port.raw_handle()))?,
            output_port,
        ))
    }
    ...
    pub fn write_data(&mut self, data: &[AddressableLEDData]) -> HALResult<()> {
        Ok(hal_call!(HAL_WriteAddressableLEDData(
            self.0,
            data.as_ptr(),
            data.len() as i32
        ))?)
    ...
    pub fn set_led_bit_timing(
        ...
        Ok(hal_call!(HAL_SetAddressableLEDBitTiming(
            self.0,
            high_time_0_ns,
            low_time_0_ns,
            high_time_1_ns,
    ...
    pub fn set_sync_time(&mut self, sync_time_us: i32) -> HALResult<()> {
        Ok(hal_call!(HAL_SetAddressableLEDSyncTime(
            self.0,
            sync_time_us
        ))?)
    }
