...
use crate::{error::{HALError, HALResult}, hal_call, wpistring::AllocatedWPIString};
...
pub fn get_joystick_name(joystick_num: i32) -> AllocatedWPIString {
    let mut name = WPI_String::default(); 
    unsafe { HAL_GetJoystickName(&mut name, joystick_num); }
    AllocatedWPIString::new(name)
}
