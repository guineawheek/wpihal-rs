use wpihal_sys::{
    HAL_CloseSPI, HAL_ConfigureSPIAutoStall, HAL_ForceSPIAutoRead, HAL_FreeSPIAuto,
    HAL_GetSPIAutoDroppedCount, HAL_GetSPIHandle, HAL_GetSPIMode, HAL_InitSPIAuto,
    HAL_InitializeSPI, HAL_ReadSPI, HAL_ReadSPIAutoReceivedData, HAL_SPIMode, HAL_SPIPort,
    HAL_SetSPIAutoTransmitData, HAL_SetSPIChipSelectActiveHigh, HAL_SetSPIChipSelectActiveLow,
    HAL_SetSPIHandle, HAL_SetSPIMode, HAL_SetSPISpeed, HAL_StartSPIAutoRate,
    HAL_StartSPIAutoTrigger, HAL_StopSPIAuto, HAL_TransactionSPI, HAL_WriteSPI,
};
...
impl SPI {
    ...
    pub fn transaction(&mut self, tx: &[u8], rx: &mut [u8]) -> i32 {
        ...
        unsafe { HAL_TransactionSPI(self.0, tx.as_ptr(), rx.as_mut_ptr(), rx.len() as i32) }
    }

    pub fn write(&mut self, tx: &[u8]) -> i32 {
        unsafe { HAL_WriteSPI(self.0, tx.as_ptr(), tx.len() as i32) }
    }

    pub fn read(&mut self, rx: &mut [u8]) -> i32 {
        unsafe { HAL_ReadSPI(self.0, rx.as_mut_ptr(), rx.len() as i32) }
    }

    pub fn set_speed(&mut self, speed: i32) {
        unsafe {
            HAL_SetSPISpeed(self.0, speed);
        }
    }

    pub fn set_spi_mode(&mut self, mode: SPIMode) {
        unsafe {
            HAL_SetSPIMode(self.0, mode);
        }
    }
    ...
    pub fn set_handle(&mut self, handle: i32) {
        unsafe {
            HAL_SetSPIHandle(self.0, handle);
        }
    }
...
impl<'a> SPIAuto<'a> {
    ...
    pub fn read_received_data(&self, data: &mut [u32], timeout: f64) -> HALResult<usize> {
        Ok(hal_call!(HAL_ReadSPIAutoReceivedData(
            self.port(),
            data.as_mut_ptr(),
            data.len() as i32,
            timeout
    ...
    pub fn configure_auto_stall(
        &self,
        cs_to_sclk_ticks: i32,
        stall_ticks: i32,
        pow_2_bytes_per_read: i32,
    ) -> HALResult<()> {
        hal_call!(HAL_ConfigureSPIAutoStall(
            self.port(),
            cs_to_sclk_ticks,
            stall_ticks,
            pow_2_bytes_per_read
        ))
    }
