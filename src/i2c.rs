use wpihal_sys::{HAL_CloseI2C, HAL_I2CPort, HAL_InitializeI2C, HAL_ReadI2C, HAL_TransactionI2C, HAL_WriteI2C};
...
impl I2C {
    ...
    pub fn write(&mut self, addr: i32, tx: &[u8]) -> i32 {
        unsafe {
            HAL_WriteI2C(self.0, addr, tx.as_ptr(), tx.len() as i32)
        }
    }

    pub fn read(&mut self, addr: i32, rx: &mut [u8]) -> i32 {
        unsafe {
            HAL_ReadI2C(self.0, addr, rx.as_mut_ptr(), rx.len() as i32)
        }
    }
