...
pub fn download_artifact_zip_to_dir(
    platform: Platform,
    dir: &Path,
    repos: &[MavenRepo],
    artifact: &Artifact,
...
pub fn stringify_path(path: &Path) -> String {
    const PLEASE_USE_UTF8: &str =
        "your file system paths are not utf8 please build this in a utf8 pathed directory";
    ...
    if let Some(sysroot) = locate_sysroot(platform, year) {
        ...
        clang_args.push(format!(
            ...
            stringify_path(
                &sysroot
                    .cpp_include()
                    .expect("can't find c++ headers in the sysroot")
            )
        ));
    ...
    match platform {
        Platform::LinuxAthena => {
            ...
            let path = if Path::new(ATHENA_SYSROOT).exists() {
                ...
                let gcc_path = which::which(format!("arm-frc{year}-linux-gnueabi-gcc"))
                    .ok()
                    .expect("Cannot locate roboRIO compiler!");
                let prospective = gcc_path
                    .parent()
                    .expect("Toolchain is not real!")
                    .join(format!("../{ATHENA_TARGET}/sysroot"));
                if prospective.exists() {
                    prospective
                } else {
                    panic!("Expected roboRIO sysroot at {prospective:?} but it does not exist!");
                }
            };
        ...
        Platform::LinuxArm64 => {
            ...
            let path = if Path::new(ARM64_SYSROOT).exists() {
                ...
                let gcc_path = which::which(format!("aarch64-bookworm-linux-gnu-gcc"))
                    .ok()
                    .expect("Cannot locate AArch64 compiler!");
                let prospective = gcc_path
                    .parent()
                    .expect("Toolchain is not real!")
                ...
                if prospective.exists() {
                    ...
                    panic!("Expected AArch64 sysroot at {prospective:?} but it does not exist!");
                }
            };
    ...
    Some(
        p.read_dir()
            ...
            .max_by(|a, b| match (a, b) {
                (Ok(v1), Ok(v2)) => {
                    let Ok(v1_str) = v1.file_name().into_string() else {
                        return std::cmp::Ordering::Less;
                    };
                    let Ok(v1_num) = v1_str.parse::<i64>() else {
                        return std::cmp::Ordering::Less;
                    };
                    let Ok(v2_str) = v2.file_name().into_string() else {
                        return std::cmp::Ordering::Greater;
                    };
                    let Ok(v2_num) = v2_str.parse::<i64>() else {
                        return std::cmp::Ordering::Greater;
                    };
                    v1_num.cmp(&v2_num)
                }
