    ...
    match platform {
        Platform::LinuxAthena => {
            ...
            let path = if Path::new(ATHENA_SYSROOT).exists() {
                ...
                let gcc_path = which::which(format!("arm-frc{year}-linux-gnueabi-gcc")).ok().expect("Cannot locate roboRIO compiler!");
        ...
        Platform::LinuxArm64 => {
            ...
            let path = if Path::new(ARM64_SYSROOT).exists() {
                ...
                let gcc_path = which::which(format!("aarch64-bookworm-linux-gnu-gcc")).ok().expect("Cannot locate AArch64 compiler!");
                let prospective = gcc_path
                    .parent()
                    .expect("Toolchain is not real!")
